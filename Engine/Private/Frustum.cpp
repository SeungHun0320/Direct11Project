...
void CFrustum::Transform_ToLocalSpace(_fmatrix WorldMatrix)
{
	...
	_vector		vPoints[8]{}, vWorldPoints[8]{};

	for (size_t i = 0; i < 8; i++)
	{
		vPoints[i] = XMVector3TransformCoord(XMLoadFloat4(&m_vOriginalPoints[i]), ProjMatrixInverse);
		vWorldPoints[i] =  XMVector3TransformCoord(vPoints[i], ViewMatrixInverse);
		XMStoreFloat4(&m_vLocalPoints[i], XMVector3TransformCoord(vWorldPoints[i], WorldMatrix));
	}

	Make_Plane(m_vLocalPoints, m_vLocalPlanes);
}
