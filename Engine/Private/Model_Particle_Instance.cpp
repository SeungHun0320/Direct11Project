...
CModel_Particle_Instance::CModel_Particle_Instance(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
    :CComponent{ pDevice, pContext }
{
}

CModel_Particle_Instance::CModel_Particle_Instance(const CModel_Particle_Instance& Prototype)
...
HRESULT CModel_Particle_Instance::Bind_Material(CShader* pShader, const _char* pConstantName, _uint iMeshIndex, TEX_TYPE eType, _uint iTextureIndex)
{
...
HRESULT CModel_Particle_Instance::Initialize_Prototype(const _wstring& strModelFilePath, const CVIBuffer_Mesh_Particle_Instance::DESC* pArg, _fmatrix PreTransformMatrix)
{
...
HRESULT CModel_Particle_Instance::Initialize(void* pArg)
{
    for (auto& pMeshInstance : m_MeshInstances)
...
HRESULT CModel_Particle_Instance::Render(_uint iMeshIndex)
{
...
void CModel_Particle_Instance::Drop(_uint iMeshIndex, _float fTimeDelta)
{
    m_MeshInstances[iMeshIndex]->Drop(fTimeDelta);
}

void CModel_Particle_Instance::Spread(_uint iMeshIndex, _float fTimeDelta)
{
...
HRESULT CModel_Particle_Instance::Ready_Meshes(ifstream& _InFile, const CVIBuffer_Mesh_Particle_Instance::DESC* pArg, _fmatrix PreTransformMatrix)
{
    ...
    for (_uint i = 0; i < m_iNumMeshes; i++)
    {
        ...
        CVIBuffer_Mesh_Particle_Instance::MESH_DESC tDesc{};
        ...
        CVIBuffer_Mesh_Particle_Instance* pMeshInstance = CVIBuffer_Mesh_Particle_Instance::Create(m_pDevice, m_pContext, &tDesc, pArg, PreTransformMatrix);
...
HRESULT CModel_Particle_Instance::Ready_Material(ifstream& _InFile)
{
    /* 이쪽에서 파일입출력해서 갯수를 먼저 받아오기  */
...
CModel_Particle_Instance* CModel_Particle_Instance::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const _wstring& strModelFilePath, const CVIBuffer_Mesh_Particle_Instance::DESC* pArg, _fmatrix PreTransformMatrix)
{
    CModel_Particle_Instance* pInstance = new CModel_Particle_Instance(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype(strModelFilePath, pArg, PreTransformMatrix)))
    {
        MSG_BOX("Failed to Created : CModel_Particle_Instance");
...
CComponent* CModel_Particle_Instance::Clone(void* pArg)
{
    CModel_Particle_Instance* pInstance = new CModel_Particle_Instance(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX("Failed to Cloned : CModel_Particle_Instance");
        Safe_Release(pInstance);
    }
...
void CModel_Particle_Instance::Free()
{
    __super::Free();
