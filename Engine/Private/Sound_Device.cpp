...
void CSound_Device::Set_Listener_Position(_fmatrix Matrix, _fvector vel)
{
    // 카메라 스테이트임으로 항상 노말라이즈 되어서 들어온다고 가정하겠음.
    FMOD_VECTOR vPos, vUp, vLook, vVel{};

    XMStoreFloat3(reinterpret_cast<_float3*>(&vUp), Matrix.r[1]);
    XMStoreFloat3(reinterpret_cast<_float3*>(&vLook), Matrix.r[2]);
    XMStoreFloat3(reinterpret_cast<_float3*>(&vPos), Matrix.r[3]);
    XMStoreFloat3(reinterpret_cast<_float3*>(&vVel), vel);
